...
import { Clock, Plane, ArrowRight, Search, Edit, CheckCircle, FileText, Trash2 } from 'lucide-react';
...
type PendingBookingRow = {
  ...
  // NOTE: booking_status viene en el select, pero no lo tipamos porque no se usa directo en UI
...
function EmissionsDashboard() {
  ...
  // Limpiar selección al cambiar pestaña (evita mezclar ids de bookings vs vouchers)
  useEffect(() => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (activeTab === 'pending') {
        ...
        const { data, error } = await supabase.from('vouchers').select('*').order('created_at', {
          ascending: false,
        });
  ...
  const filteredHistory = useMemo(() => {
    ...
    return history.filter(
      (v) => (v.invoice_id?.toLowerCase().includes(q) ?? false) || (v.client_email?.toLowerCase().includes(q) ?? false)
    );
  }, [history, searchQuery]);

  const visibleIds = useMemo(() => {
    return activeTab === 'pending' ? pending.map((b) => b.id) : filteredHistory.map((v) => v.id);
  }, [activeTab, pending, filteredHistory]);
  ...
  async function deleteSelected(ids: string[]) {
    ...
    if (activeTab === 'history') {
      ...
      setHistory((prev) => prev.filter((v) => !ids.includes(v.id)));
      return;
    }

    // activeTab === 'pending' -> bookings + dependencias
    const childDeletes = [
      supabase.from('agent_commissions').delete().in('booking_id', ids),
      supabase.from('reviews').delete().in('booking_id', ids),
      supabase.from('payment_events').delete().in('booking_id', ids),
      supabase.from('booking_itineraries').delete().in('booking_id', ids),
      supabase.from('booking_passengers').delete().in('booking_id', ids),
      supabase.from('passengers').delete().in('booking_id', ids),
    ];

    for (const req of childDeletes) {
      const { error } = await req;
      if (error) throw new Error(error.message);
    }

    const { error: bookingErr } = await supabase.from('bookings').delete().in('id', ids);
    if (bookingErr) throw new Error(bookingErr.message);

    setPending((prev) => prev.filter((b) => !ids.includes(b.id)));
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 w-full max-w-6xl mx-auto">
      ...
      <div className="flex gap-6 mb-6 border-b border-slate-200">
        <button
          className={`pb-3 font-bold text-sm transition-all flex items-center gap-2 ${
            activeTab === 'pending' ? 'border-b-2 border-brand-600 text-brand-600' : 'text-slate-500'
          }`}
        ...
        <button
          className={`pb-3 font-bold text-sm transition-all flex items-center gap-2 ${
            activeTab === 'history' ? 'border-b-2 border-brand-600 text-brand-600' : 'text-slate-500'
          }`}
          onClick={() => setActiveTab('history')}
          type="button"
        >
          <CheckCircle className="h-4 w-4" /> Historial
        </button>
      </div>
      ...
      {activeTab === 'history' && (
        <div className="mb-4 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-brand-500 text-sm shadow-sm"
          />
        </div>
      )}

      {/* ✅ Barra de acciones (debajo del buscador / tabs) */}
      {showActions && (
        <div className="mb-6 flex items-center justify-end gap-4">
          ...
          >
            <span
              className={`h-4 w-4 rounded-full border block ${
                allSelected ? 'bg-brand-600 border-brand-600' : 'bg-white border-slate-300'
              }`}
            />
          ...
          <button
            ...
            onClick={async () => {
              ...
              const ok = window.confirm(`¿Eliminar ${ids.length} elemento(s)?`);
              if (!ok) return;
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-3 py-2 text-white text-sm font-bold hover:bg-red-700"
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20 text-slate-500">Cargando...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeTab === 'pending'
            ? pending.map((booking) => (
                <div
                  key={booking.id}
                  className={`relative bg-white rounded-xl shadow-sm border p-5 flex flex-col justify-between ${
                    selectedIds.has(booking.id) ? 'border-brand-500 ring-2 ring-brand-500/20' : 'border-slate-200'
                  }`}
                >
                  {/* ✅ bolita selector */}
                  <button
                  ...
                  >
                    <span
                      className={`h-4 w-4 rounded-full border block ${
                        selectedIds.has(booking.id) ? 'bg-brand-600 border-brand-600' : 'bg-white border-slate-300'
                      }`}
                    />
                  </button>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">{norm(booking.profile)?.full_name}</p>
                  </div>

                  <button
                    type="button"
                    onClick={() => router.push(`/admin/dashboard/emission?id=${booking.id}`)}
                    className="w-full bg-[#0F2545] text-white py-2.5 rounded-lg mt-4 text-sm font-medium transition-colors hover:bg-brand-700"
                  >
                    Generar Boleto <ArrowRight className="ml-1 inline h-4 w-4" />
                  </button>
                </div>
              ))
            : filteredHistory.map((voucher) => (
                <div
                  key={voucher.id}
                  className={`relative bg-white rounded-xl shadow-sm border p-5 flex flex-col justify-between ${
                    selectedIds.has(voucher.id) ? 'border-brand-500 ring-2 ring-brand-500/20' : 'border-slate-200'
                  }`}
                >
                  {/* ✅ bolita selector */}
                  <button
                  ...
                  >
                    <span
                      className={`h-4 w-4 rounded-full border block ${
                        selectedIds.has(voucher.id) ? 'bg-brand-600 border-brand-600' : 'bg-white border-slate-300'
                      }`}
                    />
                  </button>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 truncate">{voucher.client_email}</p>
                  </div>

                  <div className="flex gap-2 mt-5">
                    <button
                      type="button"
                      onClick={() => voucher.pdf_url && window.open(voucher.pdf_url, '_blank')}
                    ...
                    <button
                      type="button"
                      onClick={() => router.push(`/admin/dashboard/emission?vid=${voucher.id}`)}
                      className="flex-1 bg-brand-600 hover:bg-brand-700 text-white py-2 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-1"
                    >
...
function EmissionForm({ bookingId, voucherId }: { bookingId?: string; voucherId?: string }) {
  ...
  const updateFlight = (index: number, field: keyof FlightSegment, value: string, isReturn: boolean = false) => {
    ...
    if (isReturn) setReturnFlights(targetArray);
    else setFlights(targetArray);
  };
  ...
  const addFlight = (isReturn: boolean = false) => {
    ...
    if (isReturn) setReturnFlights([...returnFlights, newFlight]);
    else setFlights([...flights, newFlight]);
  };

  const removeFlight = (index: number, isReturn: boolean = false) => {
    if (isReturn) setReturnFlights(returnFlights.filter((_, i) => i !== index));
  ...
  const handleEmit = async () => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('vouchers').getPublicUrl(fileName);
      ...
      if (!emailRes.ok) console.error('Error mandando correo, pero el PDF se guardó.');
  ...
  const inputClass =
    'w-full text-xs p-1.5 border border-slate-300 rounded focus:border-brand-500 outline-none uppercase placeholder:lowercase';
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-120px)] w-full gap-6 p-4">
      <div className="w-full lg:w-2/5 bg-white p-5 rounded-xl border border-slate-200 overflow-y-auto custom-scrollbar relative">
        ...
        <button onClick={() => router.push('/admin/dashboard/emission')} className="text-xs font-bold text-slate-500 mb-4" type="button">
          ← Volver al Centro
        </button>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 bg-slate-50 p-3 rounded-lg">
          <div>
            <label className="block text-[10px] font-bold text-slate-500 mb-1">RECIBO</label>
            <input type="text" value={invoiceId} className={inputClass} readOnly />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-slate-500 mb-1">FECHA</label>
            <input type="text" value={issueDate} className={inputClass} readOnly />
          </div>
        ...
        <div className="mb-6 p-3 bg-yellow-50 rounded-lg border border-yellow-100">
          <label className="flex items-center gap-2 text-[10px] font-bold text-yellow-700 mb-2 uppercase">
            <FileText className="h-3 w-3" /> Políticas del Boleto
          </label>
          <textarea
            value={policies}
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <button onClick={addPassenger} className="text-xs bg-[#0F2545] text-white px-2 py-1 rounded" type="button">
              + Añadir
            </button>
          </div>
          {passengers.map((pax, index) => (
            <div key={`pax-${index}`} className="bg-white border border-slate-200 p-3 rounded-lg mb-3 relative">
              <button onClick={() => removePassenger(index)} className="absolute top-2 right-2 text-red-500 font-bold text-xs" type="button">
                ✕
              </button>
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div className="col-span-2">
                  <label className="block text-[10px] font-bold text-slate-500">NOMBRE</label>
                  <input type="text" value={pax.fullName} onChange={(e) => updatePassenger(index, 'fullName', e.target.value)} className={inputClass} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-500">PNR</label>
                  <input type="text" value={pax.pnr} onChange={(e) => updatePassenger(index, 'pnr', e.target.value)} className={inputClass} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-brand-600">NÚMERO DE TICKET</label>
                  <input
                    type="text"
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <button onClick={() => addFlight(false)} className="text-xs bg-brand-600 text-white px-2 py-1 rounded" type="button">
              + Escala
            </button>
          </div>
          {flights.map((flight, index) => (
            <div key={`flight-out-${index}`} className="bg-slate-50 border border-slate-200 p-3 rounded-lg mb-3 relative">
              <button onClick={() => removeFlight(index, false)} className="absolute top-2 right-2 text-red-500 font-bold text-xs" type="button">
                ✕
              </button>
              <div className="grid grid-cols-3 gap-2 mt-2">
                <div className="col-span-2">
                  <label className="text-[9px] font-bold text-slate-400">AEROLÍNEA</label>
                  <input type="text" value={flight.airline} onChange={(e) => updateFlight(index, 'airline', e.target.value, false)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">VUELO</label>
                  <input
                    type="text"
                    value={flight.flightNumber}
                    onChange={(e) => updateFlight(index, 'flightNumber', e.target.value, false)}
                    className={inputClass}
                  />
                </div>
                <div className="col-span-3">
                  <label className="text-[9px] font-bold text-slate-400">FECHA</label>
                  <input type="text" value={flight.date} onChange={(e) => updateFlight(index, 'date', e.target.value, false)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">ORIGEN</label>
                  <input type="text" value={flight.origin} onChange={(e) => updateFlight(index, 'origin', e.target.value, false)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">DESTINO</label>
                  <input
                    type="text"
                    value={flight.destination}
                    onChange={(e) => updateFlight(index, 'destination', e.target.value, false)}
                    className={inputClass}
                  />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">CLASE</label>
                  <input
                    type="text"
                    value={flight.cabinClass}
                    onChange={(e) => updateFlight(index, 'cabinClass', e.target.value, false)}
                    className={inputClass}
                  />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">SALIDA</label>
                  <input
                    type="text"
                    value={flight.departure}
                    onChange={(e) => updateFlight(index, 'departure', e.target.value, false)}
                    className={inputClass}
                  />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">LLEGADA</label>
                  <input type="text" value={flight.arrival} onChange={(e) => updateFlight(index, 'arrival', e.target.value, false)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">ESTADO</label>
                  <input type="text" value={flight.status} onChange={(e) => updateFlight(index, 'status', e.target.value, false)} className={inputClass} />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <button
              ...
              className="text-xs bg-[#0F2545] hover:bg-[#1a3f7a] transition-colors text-white px-2 py-1 rounded"
              type="button"
            >
              + Añadir Retorno
            </button>
          ...
          {returnFlights.map((flight, index) => (
            <div key={`flight-ret-${index}`} className="bg-slate-50 border border-slate-200 p-3 rounded-lg mb-3 relative">
              <button onClick={() => removeFlight(index, true)} className="absolute top-2 right-2 text-red-500 font-bold text-xs" type="button">
                ✕
              </button>
              <div className="grid grid-cols-3 gap-2 mt-2">
                <div className="col-span-2">
                  <label className="text-[9px] font-bold text-slate-400">AEROLÍNEA</label>
                  <input type="text" value={flight.airline} onChange={(e) => updateFlight(index, 'airline', e.target.value, true)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">VUELO</label>
                  <input
                    type="text"
                    value={flight.flightNumber}
                    onChange={(e) => updateFlight(index, 'flightNumber', e.target.value, true)}
                    className={inputClass}
                  />
                </div>
                <div className="col-span-3">
                  <label className="text-[9px] font-bold text-slate-400">FECHA</label>
                  <input type="text" value={flight.date} onChange={(e) => updateFlight(index, 'date', e.target.value, true)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">ORIGEN</label>
                  <input type="text" value={flight.origin} onChange={(e) => updateFlight(index, 'origin', e.target.value, true)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">DESTINO</label>
                  <input
                    type="text"
                    value={flight.destination}
                    onChange={(e) => updateFlight(index, 'destination', e.target.value, true)}
                    className={inputClass}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={flight.cabinClass}
                    onChange={(e) => updateFlight(index, 'cabinClass', e.target.value, true)}
                    className={inputClass}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={flight.departure}
                    onChange={(e) => updateFlight(index, 'departure', e.target.value, true)}
                    className={inputClass}
                  />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">LLEGADA</label>
                  <input type="text" value={flight.arrival} onChange={(e) => updateFlight(index, 'arrival', e.target.value, true)} className={inputClass} />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-400">ESTADO</label>
                  <input type="text" value={flight.status} onChange={(e) => updateFlight(index, 'status', e.target.value, true)} className={inputClass} />
                </div>
              </div>
            </div>
          ))}
        ...
        <button onClick={handleEmit} disabled={isEmitting} className="w-full bg-[#FF4757] text-white py-4 rounded-xl font-bold disabled:opacity-50 mt-4" type="button">
