/**
 * @fileoverview Client-side interactive portion of the offer detail page.
 * Handles: passenger selector, date selection, booking navigation, auth gate.
 * @module app/(public)/offers/[id]/OfferDetailClient
 */
'use client';
...
export default function OfferDetailClient({ offer }: OfferDetailClientProps) {
  ...
  function handleSelectDate(date: string) {
    ...
    if (!user) {
      router.push(`/login?redirect=/offers/${offer.id}`);
      return;
    }
    router.push(`/checkout?offer=${offer.id}&date=${date}&passengers=${passengers}`);
  }
    ...
    <>
      ...
      <div className="relative bg-gradient-to-br from-brand-900 via-brand-800 to-indigo-900 py-12 lg:py-16">
        ...
        <div className="relative mx-auto max-w-6xl px-6">
          ...
          >
            <ArrowRight className="h-4 w-4 rotate-180" /> Volver a ofertas
          </Link>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {seatsLeft > 0 && (
              <Card variant="bordered">
                ...
                {seatsLeft < 9 && (
                  <p className="mt-2 text-xs text-neutral-400">Max. {seatsLeft} cupo{seatsLeft > 1 ? 's' : ''} disponibles</p>
                )}
              </Card>
            )}
            ...
            {!user && (
              <Card variant="bordered" className="bg-brand-50 border-brand-200">
                <p className="text-sm text-brand-800">
                  ...
                  <Link href={`/login?redirect=/offers/${offer.id}`} className="font-bold underline">
                    iniciar sesion
                  </Link>{' '}
