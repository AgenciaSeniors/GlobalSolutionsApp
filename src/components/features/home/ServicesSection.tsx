      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-12 text-center">
          ...
          <h2 className="mt-4 font-display text-4xl font-extrabold tracking-tight text-brand-950">
            {t('services.title')} <span className="text-[#FF4757]">{t('services.titleAccent')}</span>
          </h2>

          <p className="mt-3 text-neutral-600">{t('services.subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {SERVICES.map((s) => (
            ...
            >
              ...
              <h3 className="mt-5 text-xl font-extrabold text-brand-950">{t(s.titleKey)}</h3>
              <p className="mt-2 text-sm leading-relaxed text-neutral-600">{t(s.descKey)}</p>

              <div className="mt-6">
                <Link href={s.href}>
                  <Button className="gap-2 bg-[#FF4757] text-white hover:bg-[#ff2f43]">{t('services.cta')}</Button>
                </Link>
              </div>
