      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 font-display text-4xl font-extrabold tracking-tight">
            {t('about.title')} <span className="text-coral">{t('about.titleAccent')}</span>
          </h2>

          <p className="mt-4 text-base leading-relaxed text-white/80">{t('about.subtitle')}</p>

          <div className="mt-7 flex flex-wrap justify-center gap-4">
            <Link href={ROUTES.ABOUT}>
              <Button className="gap-2">{t('about.cta')}</Button>
            </Link>
          </div>
        </div>
        ...
        <div className="mx-auto mt-12 grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-3">
          {FEATURES.map((f) => (
            ...
            >
              ...
              <p className="mt-4 text-lg font-bold text-white">{t(f.titleKey)}</p>
              <p className="mt-2 text-sm leading-relaxed text-white/75">{t(f.descKey)}</p>
            </div>
          ))}
        </div>
      </div>
